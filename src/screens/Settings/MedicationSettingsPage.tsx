...
const MedicationSettingsPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!userSettings) {
      userSettingsApi.getUserSettings().then(settings => {
        console.log('取得したuserSettings:', settings);
  ...
  const handleSave = async () => {
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 100));
      await userSettingsApi.updateMedicationEffectMode(isEffectModeOn);
      // zustandストアも更新
      setUserSettings({
        ...userSettings,
        effect_start_time: localConfig.defaultTime,
        ...
        <div className="flex items-center justify-between">
          ...
          <label className="relative inline-flex items-center w-11 h-6 cursor-pointer">
            <input
              ...
              className="sr-only"
            />
            <div className={`w-11 h-6 rounded-full transition-colors duration-200 ${isEffectModeOn ? 'bg-blue-600' : 'bg-gray-200'}`}></div>
            <span
              className={`absolute left-1 top-1 w-4 h-4 bg-white rounded-full shadow transition-transform duration-200 ${isEffectModeOn ? 'translate-x-5' : ''}`}
            ></span>
          </label>
        </div>
        {/* アコーディオン：ONのときだけ表示 */}
